...
export const AdminDashboard = ({ currentUser, onNotify, onLogout }: Props) => {
  const [active, setActive] = useState('Enrollment Approval');
  ...
  const render = () => {
    ...
    if (active === 'Account Creation') {
      return table('Account Creation / Management', ['ID', 'Name', 'Role', 'Actions'],
        accounts.list.map((a) => <tr key={a.id} style={{ borderTop: '1px solid #eef1f6' }}><td style={{ padding: 12 }}>{a.id}</td><td style={{ padding: 12 }}>{a.name}</td><td style={{ padding: 12 }}>{a.role}</td><td style={{ padding: 12 }}><div style={{ display: 'flex', gap: 6 }}><button style={ghost} onClick={() => { const v = prompt('Edit name', a.name); if (v) { accounts.update(a.id, { name: v }); onNotify('Account updated'); } }}>Edit</button><button style={{ ...ghost, color: '#b91c1c' }} onClick={() => { accounts.remove(a.id); onNotify('Account deleted'); }}>Delete</button></div></td></tr>),
    ...
    if (active === 'Fee Structure' || active === 'Billing & Invoices' || active === 'Teacher Records' || active === 'Room Allocation' || active === 'Subject Offering' || active === 'Broadcast Messaging' || active === 'System Announcements') {
      ...
      return table(active, ['Name', 'Detail', 'Actions'],
        col.list.map((r) => <tr key={r.id} style={{ borderTop: '1px solid #eef1f6' }}><td style={{ padding: 12 }}>{r.name}</td><td style={{ padding: 12 }}>{r.role}</td><td style={{ padding: 12 }}><div style={{ display: 'flex', gap: 6 }}><button style={ghost} onClick={() => { const v = prompt('Edit', r.name); if (v) { col.update(r.id, { name: v }); onNotify('Updated'); } }}>Edit</button><button style={{ ...ghost, color: '#b91c1c' }} onClick={() => { col.remove(r.id); onNotify('Deleted'); }}>Delete</button></div></td></tr>),
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f3f5f9', fontFamily: 'Inter,system-ui,sans-serif' }}>
      <header style={{ height: 68, background: '#fff', borderBottom: '1px solid #e5e9f0', display: 'flex', alignItems: 'center', padding: '0 20px', gap: 14, position: 'sticky', top: 0, zIndex: 5 }}>
        ...
        <span style={{ background: '#e8f1ff', color: '#1d5fc2', fontSize: 12, fontWeight: 800, borderRadius: 8, padding: '5px 10px' }}>ADMIN</span><span style={{ color: '#8a94a6', fontSize: 13 }}>{route}</span>
        <div style={{ marginLeft: 'auto' }}><div style={{ width: 36, height: 36, borderRadius: '50%', background: NAVY, color: '#fff', display: 'grid', placeItems: 'center', fontWeight: 800 }}>A</div></div>
      </header>
