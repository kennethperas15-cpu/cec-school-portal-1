...
const EnrollmentPage = ({ page, onNotify, hasEnrollment }: { page: string; onNotify: (text: string) => void; hasEnrollment: boolean }) => {
  const [selectedSections, setSelectedSections] = useState(['Data Structures - BSIT-3A', 'Data Structures - BSIT-3B']);
  const [documents, setDocuments] = useState<Record<string, string>>({ COR: 'cor.pdf', 'PSA Birth Certificate': 'psa.pdf' });
  const [submitted, setSubmitted] = useState(false);
  const sections = [
    ['Data Structures - BSIT-3A', 'Capacity 30/35'],
    ['Data Structures - BSIT-3B', 'Capacity 32/35'],
    ['Web Development - BSIT-3A', 'Capacity 25/40'],
  ];
  const toggleSection = (section: string) => setSelectedSections((current) => current.includes(section) ? current.filter((item) => item !== section) : [...current, section]);
  ...
  if (page === 'Document Submission' && !hasEnrollment) return <section className="academic-card enrollment-card"><h1>Document Submission (Requirements Upload)</h1><div className="empty-state enrollment-empty"><span className="empty-icon">○</span><strong>No requirements submitted yet</strong><p>Documents will appear here after the student begins the enrollment process.</p></div></section>;
  if (page === 'Status Tracker' && !hasEnrollment) return <section className="academic-card enrollment-card"><h1>Enrollment Status Tracker</h1><div className="empty-state enrollment-empty"><span className="empty-icon">○</span><strong>No enrollment application yet</strong><p>Your enrollment status will appear here after you submit the online enrollment form.</p></div></section>;
  ...
  if (page === 'Section Selection') return <section className="academic-card enrollment-card"><h1>Section / Subject Selection</h1><div className="section-list">{sections.map(([name, capacity]) => <label className={selectedSections.includes(name) ? 'section-option selected' : 'section-option'} key={name}><input type="checkbox" checked={selectedSections.includes(name)} onChange={() => toggleSection(name)} /><span><strong>{name}</strong><small>{capacity}</small></span></label>)}</div><button className="primary-button" onClick={() => onNotify(`${selectedSections.length} section${selectedSections.length === 1 ? '' : 's'} selected`)}>Save Section Selection <span>→</span></button></section>;
  ...
  return <section className="academic-card enrollment-card"><h1>Enrollment Status Tracker</h1><div className="status-steps">{['Applied', 'Documents Verified', 'Assessed', 'Paid', 'Approved', 'Enrolled'].map((step, index) => <div className={index < 3 ? 'status-step complete' : 'status-step'} key={step}><span>{index < 2 ? '✓' : index + 1}</span><b>{step}</b></div>)}</div><div className="status-message">Current: <strong>Assessed</strong> · Next: complete payment to proceed.</div></section>;
};
